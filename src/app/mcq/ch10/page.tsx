...
import { ChevronLeft, Search, Bookmark, BookmarkCheck, CheckCircle2, XCircle, ChevronDown, ChevronUp, Code2, Lightbulb, LayoutGrid, List } from "lucide-react";
import Link from "next/link";
import { mcqCh10, descriptiveCh10 } from "@/data/mcq-ch10";
...
export default function Ch10Page() {
  ...
  const [answers, setAnswers] = useState<(number | null)[]>(new Array(mcqCh10.length).fill(null));
  ...
  const [bookmarks, setBookmarks] = useState<number[]>(() => {
    try {
      const saved = localStorage.getItem("mcq-ch10-bookmarks");
  ...
  const toggleBookmark = (id: number) => {
    ...
    setBookmarks(next);
    localStorage.setItem("mcq-ch10-bookmarks", JSON.stringify(next));
  };

  const filteredMcq = useMemo(() => {
    if (!search) return mcqCh10;
    return mcqCh10.filter(m => 
  ...
  if (finished) {
    const pct = Math.round((score / mcqCh10.length) * 100);
    return (
      <div className="max-w-[640px] mx-auto px-4 md:px-8 pt-20 pb-16 text-center">
        <div className="card p-10">
          <div className="w-16 h-16 rounded-xl bg-white/[0.06] flex items-center justify-center mx-auto mb-5">
            <Bookmark size={30} className="text-white" />
          </div>
          <h2 className="text-2xl font-bold text-white mb-2">Practice Complete!</h2>
          <div className="text-6xl font-bold text-white mb-1">{score}<span className="text-[#5a5a5a]">/{mcqCh10.length}</span></div>
          ...
          <div className="flex items-center justify-center gap-2 mb-6">
            <div className="w-full max-w-xs h-1.5 rounded-full bg-white/[0.06]">
              <div className="h-full rounded-full bg-gradient-to-r from-gray-300 to-gray-500 transition-all" style={{ width: `${pct}%` }} />
            </div>
          </div>
          <button onClick={() => { setCurrent(0); setSelected(null); setShowResult(false); setScore(0); setFinished(false); setAnswers(new Array(mcqCh10.length).fill(null)); }} className="px-6 py-3 rounded-lg bg-white text-black text-sm font-medium hover:bg-white/[0.80] transition-colors">
            Restart Practice
          </button>
        </div>
      </div>
    ...
    <div className="max-w-[960px] mx-auto px-4 md:px-8 pt-6 md:pt-10 pb-16">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <h1 className="text-2xl font-bold text-white">Chapter 10: Mongoose</h1>
          <p className="text-sm text-[#8a8a8a] mt-0.5">40 Questions · Schema, Model, CRUD, Validation, Population</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        ...
        {mode === "solution" && (
          <div className="relative flex-1 max-w-xs">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search questions..." className="w-full h-9 pl-9 pr-3 rounded-lg bg-white/[0.04] border border-[rgba(255,255,255,0.08)] text-sm text-white placeholder:text-[#5a5a5a] outline-none focus:border-white transition-colors" />
          </div>
        )}
        {mode === "interactive" && (
          <div className="flex items-center gap-2 ml-auto">
            <span className="text-xs text-[#5a5a5a]">{current + 1} / {mcqCh10.length}</span>
            <div className="w-24 h-1.5 rounded-full bg-white/[0.06]">
              <div className="h-full rounded-full bg-gradient-to-r from-gray-300 to-gray-500 transition-all" style={{ width: `${((current + 1) / mcqCh10.length) * 100}%` }} />
            </div>
          </div>
        )}
      </div>
          ...
          {showResult && (
            <div className="flex justify-end mt-4">
              <button onClick={next} className="px-5 py-2.5 rounded-lg bg-white text-black text-sm font-medium hover:bg-white/[0.80] transition-colors">
                {current < mcqCh10.length - 1 ? "Next Question" : "See Results"}
              </button>
            </div>
          )}
        </div>
